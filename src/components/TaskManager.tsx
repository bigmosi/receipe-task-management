  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6 px-2 md:px-0 bg-gradient-to-br from-orange-50 via-yellow-100 to-pink-100 min-h-screen pb-12">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 pt-8">
        <div>
          <h2 className="text-3xl font-extrabold text-pink-700 drop-shadow">Task Management</h2>
          <p className="text-pink-500 mt-1">Organize your tasks and stay productive</p>
        </div>
        <button
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-2 z-50">
          <div className="bg-gradient-to-br from-pink-50 via-orange-50 to-yellow-50 rounded-xl shadow-2xl w-full max-w-md">
            ...
            <form onSubmit={handleSubmit} className="p-4 sm:p-6 space-y-4">
              <div>
                <label className="block text-sm font-medium text-pink-700 mb-2">Title</label>
                <input
                  type="text"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-pink-700 mb-2">Priority</label>
                  <select
                ...
                <div>
                  <label className="block text-sm font-medium text-pink-700 mb-2">Due Date</label>
                  <input
                    type="date"
      ...
      {filteredTasks.length > 0 ? (
        <div className="space-y-4">
          {filteredTasks.map((task) => {
            ...
            return (
              <div
                key={task.id}
                className={`rounded-xl border p-4 sm:p-6 hover:shadow-lg transition-all duration-200 ${getTaskCardBorder(task)}`}
              >
                  ...
                  <div className="flex items-start space-x-4 flex-1">
                    ...
                    <div className="flex-1">
                      <div className="flex flex-wrap items-center gap-3 mb-2">
                        <h3 className={`text-lg font-semibold ${task.completed ? 'text-pink-300 line-through' : 'text-pink-900'}`}>
                          {task.title}
                        </h3>
                      ...
                      {task.description && (
                        <p className={`text-pink-500 mb-3 ${task.completed ? 'line-through' : ''}`}>
                          {task.description}
                        </p>
                      )}
        ...
        <div className="bg-gradient-to-br from-pink-50 via-orange-50 to-yellow-50 rounded-lg border border-pink-100 p-12 text-center">
          ...
          <h3 className="text-lg font-medium text-pink-700 mb-2">No tasks found</h3>
